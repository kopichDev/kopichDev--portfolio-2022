$(function(){
    ...
    $('.info-tabs__item').on('click', function(e) { // вызываем функцию при одинарном клике на класс

        e.preventDefault(); // устанавливаем поведение для класса - по умолчанию

        $('.info-tabs__item').removeClass('info-tabs__item--active'); // удаляем активный класс у выбранного элемента (делаем невидимым)
        $(this).addClass('info-tabs__item--active'); // добавляем активный класс для выбранного элемента (мгновенно после этого делаем видимым)

        $('.info-tabs__content').removeClass('info-tabs__content--active'); // аналогично удаляем класс, делаем невидимым
        $($(this).attr('href')).addClass('info-tabs__content--active'); // для выбранной ссылки добавляем класс видимости

    });

    $('.info-slider, .clients-slider').slick({
        fade: true, // эффект переключения с затуханием с одного на другое
        speed: 500,   // скорость переключения, чем больше, тем медленнее
